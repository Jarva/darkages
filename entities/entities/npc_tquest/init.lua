...
function ENT:AcceptInput( Name, Activator, ply )	
	...
	local quest = self:GetNWString("quest")
	...
	if (ply.data.inventory[item] == nil or ply.data.inventory[item] == 0) then
	
	data.qstate = qstate
	
	else
	
	data.qstate = "Completed"
	
	end
	
	data.quest = quest
	data.task = Quest[quest].task
	data.item = item
