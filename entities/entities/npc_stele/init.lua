...
function STeleport(ply)
	for k,v in pairs(ents.FindInSphere(ply:GetPos(),300)) do
		if (v:GetClass() == "npc_stele") then
			ent = v				
		
			if (ply.data.skills[ent:GetNWString("skill")] < tonumber(ent:GetNWInt("level"))) then
			
				HudText(ply,"You need lvl "..ent:GetNWInt("level").." "..ent:GetNWString("skill").." to enter")
...
concommand.Add("DA_STele",STeleport)
...
function ENT:KeyValue(key, value)
	...
	self:SetNWInt("level",value)
	
	end
	
	if (key == "skill") then
	
	self:SetNWString("skill",value)
