...
function ENT:AcceptInput( Name, Activator, ply )	
	...
	if Name == "Use" and ply:IsPlayer() then
		...
		if (Arena[self:GetNWString("arena")].players[1] == ply:SteamID()) then x = 1 end
		if (Arena[self:GetNWString("arena")].status == "Action") then x = 2 end
		
		umsg.Start("arenamenu",ply)
		umsg.String(self:GetNWString("arena"))
		umsg.Short(x)
		umsg.End()
